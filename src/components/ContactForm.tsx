  ...
  return (
    <section id="contact" className="bg-[#050505] border-t border-white/10 text-white py-24 px-6 md:px-12 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {/* Dynamic Inbox Toggle bar (Martin's hidden Lead Inbox for demonstration) */}
        <div className="mt-16 pt-8 border-t border-white/10 font-mono text-xs text-neutral-500">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center space-x-2">
              <BadgeInfo className="w-4 h-4 text-cyan-400 animate-pulse" />
              <span>Are you Martin Walker reviewing incoming leads? Open dashboard:</span>
            </div>
            
            <button
              onClick={() => setIsAdminView(!isAdminView)}
              className={`px-4 py-2 border font-mono text-[10px] uppercase font-bold tracking-widest transition ${
                isAdminView 
                  ? "bg-cyan-950 text-cyan-400 border-cyan-900" 
                  : "bg-neutral-950 text-neutral-400 border-white/10 hover:text-white"
              } cursor-pointer`}
            >
              {isAdminView ? "Close Leads Console" : "[Open Leads Inbox]"}
            </button>
          </div>
          ...
          <AnimatePresence>
            {isAdminView && (
              <motion.div
                ...
                className="mt-6 border border-white/10 bg-neutral-950 rounded overflow-hidden"
              >
                <div className="px-6 py-4 bg-[#050505] border-b border-white/10 flex items-center justify-between">
                  <span className="font-bold text-neutral-200 flex items-center space-x-2 font-mono">
                    <Inbox className="w-4 h-4 text-cyan-400" />
                    <span>MARKETNI LEADS SYSTEM [CONSOLE]</span>
                  </span>
                  <span className="text-[9px] text-cyan-400 font-bold bg-[#050505] px-2 py-0.5 border border-cyan-900/40">
                ...
                <div className="divide-y divide-white/5">
                  {leads.length === 0 ? (
                    <div className="p-8 text-center text-neutral-600">
                      No customer inquiries compiled yet. Fill out the contact form above to test the system!
                    </div>
                  ) : (
                    leads.map((lead) => (
                      <div 
                        key={lead.id} 
                        className={`p-4 gap-4 flex flex-col sm:flex-row justify-between items-start transition duration-200 ${
                          lead.isHandled ? "opacity-60 bg-neutral-950/30" : "bg-neutral-900/20"
                        }`}
                      >
                        <div className="space-y-2 flex-grow">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="font-bold text-neutral-200">{lead.senderName}</span>
                            <span className="text-neutral-500">({lead.senderEmail})</span>
                            <span className="text-[10px] bg-cyan-950 text-cyan-400 px-2 py-0.5 border border-cyan-900/20">
                              {lead.topic}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center space-x-1.5 text-neutral-400">
                            ...
                            <span className="text-[10px] text-neutral-500">{lead.timestamp}</span>
                          </div>

                          <p className="text-xs text-neutral-300 font-sans leading-relaxed pl-3 border-l-2 border-white/10 italic font-light">
                            &quot;{lead.messageText}&quot;
                          </p>
                        </div>
                        ...
                        <div className="flex items-center space-x-2 shrink-0 self-end sm:self-start">
                          <button
                            onClick={() => toggleHandleLead(lead.id)}
                            className={`px-3 py-1 text-[9px] uppercase font-bold transition-all border ${
                              lead.isHandled 
                                ? "bg-cyan-950/40 text-cyan-400 border-cyan-900/60" 
                                : "bg-neutral-950 hover:bg-[#050505] text-neutral-450 border-white/10"
                            } cursor-pointer`}
                          >
                            {lead.isHandled ? "Handled" : "Mark Handled"}
                          </button>
                          <button
                            onClick={() => deleteLead(lead.id)}
                            className="p-1 text-red-500 border border-white/10 hover:border-red-950 bg-neutral-950 hover:text-red-400 transition cursor-pointer"
                            title="Delete Lead"
                          >
