...
export default function Hero({ onScrollToSection }: HeroProps) {
  // Use the exact path returned by the image-generation tool
  const martinPortraitUrl = "/src/assets/images/martin_walker_1781790890216.jpg";
  ...
  return (
    <section id="hero" className="relative min-h-screen bg-[#050505] text-white overflow-hidden flex flex-col justify-between">
      ...
      {/* Main Hero Container */}
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-12 md:py-20 lg:py-24 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center flex-grow">
        ...
        >
          <div className="relative group max-w-sm w-full">
            ...
            <div className="absolute -inset-1.5 bg-gradient-to-r from-cyan-400 to-teal-500 rounded-lg blur opacity-20 group-hover:opacity-45 transition duration-1000 group-hover:duration-205" />
            ...
            <div className="relative bg-neutral-900 overflow-hidden border border-white/10 rounded-md">
              <img
                src={martinPortraitUrl}
                alt="Martin Walker Portrait - Senior Marketing Consultant"
                referrerPolicy="no-referrer"
                className="w-full aspect-[3/4] object-cover filter contrast-[1.05] grayscale-[20%] group-hover:scale-105 transition-transform duration-700"
              />
              
              <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black via-black/40 to-transparent p-6 pt-16">
                <span className="font-mono text-xs text-cyan-400 tracking-widest block uppercase font-bold mb-1">
                  Founder & Principal
                </span>
        ...
        >
          ...
          <div className="inline-flex items-center space-x-2 bg-neutral-900 border border-white/10 px-3.5 py-1.5 rounded-full">
            <Sparkles className="w-4 h-4 text-cyan-400 animate-pulse" />
            <span className="font-mono text-[10px] text-neutral-300 font-bold tracking-wider">
              CREATIVE AGENCY MEETS TECH-DRIVEN PERFORMANCE
            </span>
          </div>

          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black tracking-tight text-white leading-none">
            I BRING THE STRATEGY OF <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 via-cyan-450 to-teal-400">GLOBAL SENSATIONS</span> TO MAIN STREETS.
          </h1>

          <div className="space-y-4 max-w-2xl">
            <p className="text-lg text-neutral-350 font-sans leading-relaxed">
              Hey, I'm <strong className="text-white font-bold">Martin Walker</strong>. 
              After a decade designing high-acquisition engines for mid-market behemoths and hot scaleups, I noticed something vital: local business owners get left behind with template agencies selling generic social slop.
            </p>
            <p className="text-neutral-400 font-sans leading-relaxed">
              I founded <strong>Marketni</strong> to bring elite, modern, multi-channel growth consultancy straight to you. We couple hyper-targeted Local SEO and high-retention copy with sleek, high-tech systems built to capture local intent the second it flashes. No generic packages, no fluff. Just pure, measurable consulting that scales local brands.
            </p>
          </div>

          {/* Quick Metrics */}
          <div className="grid grid-cols-3 gap-4 border-t border-b border-white/10 py-6 font-mono text-left">
            <div>
              <div className="flex items-center space-x-1">
                <TrendingUp className="w-4 h-4 text-cyan-400" />
                <span className="text-xl sm:text-2xl font-bold tracking-tight text-white">&pound;18M+</span>
              </div>
              <span className="text-[10px] text-neutral-500 uppercase tracking-wider block mt-1">Client Growth Generated</span>
            </div>
            <div>
              <div className="flex items-center space-x-1">
                <Award className="w-4 h-4 text-cyan-400" />
                <span className="text-xl sm:text-2xl font-bold tracking-tight text-white">100%</span>
              </div>
              <span className="text-[10px] text-neutral-500 uppercase tracking-wider block mt-1">Local Growth Focus</span>
            </div>
            <div>
              <div className="flex items-center space-x-1">
                <Compass className="w-4 h-4 text-cyan-400" />
                <span className="text-xl sm:text-2xl font-bold tracking-tight text-white">3.8x</span>
              </div>
              <span className="text-[10px] text-neutral-500 uppercase tracking-wider block mt-1">Avg Search Visibility Increase</span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
            ...
            <button
              onClick={() => onScrollToSection("contact")}
              className="px-8 py-4 bg-neutral-900 hover:bg-neutral-800 border border-white/10 text-white font-mono text-xs font-bold tracking-widest duration-200 cursor-pointer text-center"
            >
              BOOK CONSULT & COLLAB
            </button>
          </div>
